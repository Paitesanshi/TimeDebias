...
def writer_csv(result_dict,dataset,model,task,robust,sr):
    ...
    rootPath = os.path.join(rootPath, 'sample')
    if not os.path.exists(rootPath):
        os.makedirs(rootPath)
    filename=model+"_"+dataset+"_"
    if robust:
        filename=filename+"rd_"+task
    else:
...
if __name__ == '__main__':
    ...
    for item_v in config_dict['sample_rate']:

        config['sample_rate']=item_v
        
        result = objective_function(config_dict=config, config_file_list=config_file_list,saved=True)
        result=result["test_result"]
        writer_csv(result, args.dataset, args.model,task,False,item_v)
        rmse = result['rmse']
        if rmse < best_rmse:
            best_item_v = item_v
    ...
    writer_csv(result_dict, args.dataset, args.model,task,False,beest_item_v)
