...
from recbole.model.general_recommender import IPSV, IPST
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        if self.config['task'] == 'ps':
            if self.model.__class__.__name__ == 'PMF':
                ...
                #     p = self.model(user, item)
                #     for i in range(len(user)):
                #         pui[(user[i].data, item[i].data)] = p[i].data
                # np.save('./init_ps/item_ps.npy', dict)
                ...
                #     user = interaction['user_id']
                #     item = interaction['item_id']
                #     p=self.model.get_p(interaction)
                #     #p = mod(user, item)
                #     p=mod.predict(interaction)
                # puit = {}
                # sig = float(self.config['sig'])
                # for batch_idx, interaction in enumerate(train_data):
                #     interaction = interaction.to(self.config['device'])
                #     user = interaction['user_id']
                #     item = interaction['item_id']
                #     ti = interaction['timestamp']
                #     pui = self.model.predict(interaction)
                #     for i in range(len(user)):
                #         po = -(ti[i] - pui[i]) * (ti[i] - pui[i]) / (2 * sig * sig)
                #         pt = 1 / (math.sqrt(2 * math.pi) * sig) * math.exp(po)
                #         puit[(user[i].data, item[i].data, ti[i].data)] = pt
                #         # pui[(user, item)] = pui
                me = self.model.__class__.__name__.lower()
                filepath = './init_ps/time_' + me + '_' + str(self.config['K']) + '_' + str(self.config['T']) + '.pth'
                torch.save(self.model, filepath)
            # filepath = './init_ps/time_' + me + '_' + str(self.config['K']) + '.npy'
...
class GANTrainer(AbstractTrainer):
    ...
    def __init__(self, config, model, model_g, model_dv):
        super(GANTrainer, self).__init__(config, model)

        self.model_g = model_g
        self.model_dv = model_dv
        self.logger = getLogger()
        self.tensorboard = get_tensorboard(self.logger)
        self.wandblogger = WandbLogger(config)
        self.learner = config['learner']
        ...
        self.train_loss_dict = dict()
        self.optimizer = self._build_optimizer()
        self.optimizer_g = self.build_optimizer(model=self.model_g)
        self.optimizer_dv = self.build_optimizer(model=self.model_dv)
        self.eval_type = config['eval_type']
        self.eval_collector = Collector(config)
        self.evaluator = Evaluator(config)
        self.item_tensor = None
        self.tot_item_num = None
        self.num_dosage_samples = int(config['num_time_sample'])
    ...
    def build_optimizer(self, model, **kwargs):
        ...
        Returns:
            torch.optim: the optimizer
        """
        params = kwargs.pop('params', model.parameters())
        learner = kwargs.pop('learner', self.learner)
        learning_rate = kwargs.pop('learning_rate', self.learning_rate)
        weight_decay = kwargs.pop('weight_decay', self.weight_decay)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        loss_func_dv = None
        loss_func = loss_func or self.model.calculate_loss
        loss_func_g = loss_func_g or self.model_g.calculate_loss
        loss_func_dv = loss_func_dv or self.model_dv.calculate_loss
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            for kk in range(2):
                ...
                losses = loss_func_g(interaction)
                if isinstance(losses, tuple):
                    loss = sum(losses)
                    loss_tuple = tuple(per_loss.item() for per_loss in losses)
                    total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
                    ...
                    total_loss = losses.item() if total_loss is None else total_loss + losses.item()
                self._check_nan(loss)
                for key in self.model_g.g_outcome.keys():
                    self.model.g_outcome[key] = self.model_g.g_outcome[key].detach()
                    self.model_dv.g_outcome[key] = self.model_g.g_outcome[key].detach()
                d_t_loss = self.model.calculate_loss(interaction)
                d_v_loss = self.model_dv.calculate_loss(interaction)
                d_t_logits = self.model.d_t_logits
                d_v_logits = self.model_dv.d_v_logits
                d_combined_prob = torch.sigmoid(d_t_logits) * torch.sigmoid(
                    torch.unsqueeze(d_v_logits, dim=-1).repeat(1, 1, self.num_dosage_samples))

                d_combined_loss = -torch.mean(
                    self.model_g.vt_mask * torch.log(d_combined_prob + 1e-7) + (1.0 - self.model_g.vt_mask) * torch.log(
                        1.0 - d_combined_prob + 1e-7))
                loss_com = loss + d_combined_loss
                loss_com.backward()
                if self.clip_grad_norm:
                    clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            ...
            for kk in range(1):
                for key in self.model_g.g_outcome.keys():
                    ...
                    self.model_dv.g_outcome[key] = self.model_g.g_outcome[key].detach()
                self.optimizer.zero_grad()
                losses_t = loss_func(interaction)
                if isinstance(losses_t, tuple):
                    loss_t = sum(losses_t)
                    loss_tuple = tuple(per_loss.item() for per_loss in losses_t)
                    total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
                else:
                    loss_t = losses_t
                    total_loss = losses_t.item() if total_loss is None else total_loss + losses_t.item()
                self._check_nan(loss_t)
                loss_t.backward()
                if self.clip_grad_norm:
                    clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
                self.optimizer.step()
                ...
                losses = loss_func_dv(interaction)
                if isinstance(losses, tuple):
                    loss = sum(losses)
                    loss_tuple = tuple(per_loss.item() for per_loss in losses)
                    total_loss = loss_tuple if total_loss is None else tuple(map(sum, zip(total_loss, loss_tuple)))
                ...
                # d_t_logits=self.model.d_t_logits
                # d_v_logits=self.model_dv.d_v_logits
                # d_combined_prob = torch.sigmoid(d_t_logits) * torch.sigmoid(torch.unsqueeze(d_v_logits, dim=-1).repeat(1, 1, self.num_dosage_samples))
                #
                # d_combined_loss = -torch.mean(
                #     self.model_g.vt_mask * torch.log(d_combined_prob + 1e-7) + (1.0 - self.model_g.vt_mask)* torch.log(
                # 1.0 - d_combined_prob + 1e-7))
    ...
    def _neg_sample_batch_eval(self, batched_data):
        ...
        batch_size = interaction.length
        if batch_size <= self.test_batch_size:
            origin_scores = self.model_g.predict(interaction.to(self.device))
        else:
            origin_scores = self._spilt_predict(interaction, batch_size)
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        if load_best_model:
            ...
            self.model_g.load_other_parameter(checkpoint.get('other_parameter'))
            message_output = 'Loading model_g structure and parameters from {}'.format(checkpoint_file)
            self.logger.info(message_output)
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            self.eval_collector.eval_batch_collect(scores, interaction, positive_u, positive_i)
        self.eval_collector.model_collect(self.model_g)
        struct = self.eval_collector.get_data_struct()
        result = self.evaluator.evaluate(struct)
        self.wandblogger.log_eval_metrics(result, head='eval')
