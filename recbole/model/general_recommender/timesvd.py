...
class TimeSVD(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.bu = torch.zeros(self.n_users).to(self.device)
        self.alpha_u = torch.zeros(self.n_users).to(self.device)
        self.y = torch.ones((self.n_items, self.embedding_size)).to(self.device)
        self.tu = torch.zeros(self.n_users).to(self.device)

        self.bias_item_bin = torch.zeros((self.n_items, self.item_bin_size)).to(self.device)
        self.time_matrix = (
            dataset.inter_matrix(form="csr", value_field="timestamp")
            .astype(np.float32)
            .toarray()
        )
        self.btday = torch.zeros(self.maxday_cat_code + 1).to(self.device)
        self.bcu = torch.zeros(self.n_users).to(self.device)
        self.wcu = torch.zeros(self.maxday_cat_code + 1).to(self.device)
        self.maxday_cat = torch.zeros(self.maxday_cat_code).long().to(self.device)
