...
class ComputeSimilarity:
    ...
    def __init__(self, dataMatrix, topk=100, shrink=0, normalize=True):
        ...
        #self.timeMatrix=timematrix.copy()


        # self.dataMatrix=self.dataMatrix*self.ft
    def compute_similarity(self, method, block_size=100):
        ...
        # csr_ft=csr_matrix(self.ft)
        # self.dataMatrix=self.dataMatrix.multiply(csr_ft)
        ...
        while start_block < end_local:
            ...
            for index_in_block in range(this_block_size):
                ...
                #this_line_time=this_block_time[:, index_in_block]
                Index = index_in_block + start_block
                this_line_weights[Index] = 0.0
                #this_line_weights=this_line_weights*this_line_time
...
class TAItemKNN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(TAItemKNN, self).__init__(config, dataset)
        ...
        self.time_matrix = dataset.inter_matrix(form='csr',value_field='timestamp').astype(np.float32)
        self.n_rows, self.n_columns = self.time_matrix.shape
        self.ft = np.zeros((self.n_rows, self.n_columns), dtype=np.float32)
        self.timedata = self.time_matrix.toarray()
        # for i in range(self.n_rows):
        #     t0 = 1e10
        #     for j in range(self.n_columns):
        #         t0 = min(timedata[i][j], t0)
        #     for j in range(self.n_columns):
        #         self.ft[i][j] = np.exp(-(timedata[i][j] - t0))

        shape = self.interaction_matrix.shape
        assert self.n_users == shape[0] and self.n_items == shape[1]
        self.sim= ComputeSimilarity(self.interaction_matrix, topk=self.k,
                                      shrink=self.shrink)
        _, self.w =self.sim.compute_similarity('item')
        # csr_ft = csr_matrix(self.ft)
        # self.w=self.w.multiply(csr_ft)
        self.pred_mat = self.interaction_matrix.dot(self.w).tolil()
        self.pred_mat /= np.sum(self.w.toarray())
    ...
    def predict(self, interaction):
        ...
        time = time.cpu().numpy().astype(float)
        result = []

        for index in range(len(user)):
            uid = user[index]
            iid = item[index]
            wt=np.exp(-(time[index]-self.timedata[uid][iid]))
            score = self.pred_mat[uid, iid]*wt
            result.append(score)
