...
def save_example():
    ...
    run_recbole(model='BPR', dataset='ml-100k_origin', config_dict=config_dict)
...
def load_example():
    ...
    config, model, dataset, train_data, valid_data, test_data = load_data_and_model(
        model_file='../saved/BPR-Aug-20-2021_03-32-13.pth',
        dataset_file='../saved/ml-100k_origin-dataset.pth',
    )

    # Dataset is neither created nor loaded, and split dataloaders are loaded from file.
    config, model, dataset, train_data, valid_data, test_data = load_data_and_model(
        model_file='../saved/BPR-Aug-20-2021_03-32-13.pth',
        dataloader_file='../saved/ml-100k_origin-for-BPR-dataloader.pth',
    )
