...
#     print('testset_num:', len(testset))
#
#     data.to_csv(data_name + "_full.csv", header=True, index=False, sep='\t')
#     trainset.to_csv(data_name + '/' + data_name + '.train.inter', header=True, index=False, sep='\t')
#     testset.to_csv(data_name + "_test_bias.csv", header=True, index=False, sep='\t')
#
#
...
#         block = get_block(nt)
#         ids.append(block)
#     data.insert(data.shape[1], 'wday:float', ids)
...
#     split_fulldata_TO2('ml_100k_week.inter')
#     sampling_from_biasData(full_path='ml_100k_week.inter', bias_path='ml_test_bias.csv')
...
def judge_userID(train_path='', test_path=''):  # 判断训练集中的用户是否都有测试数据
    ...
    for index, row in train_data.iterrows():
        ...
        if train_cnt[term] > max_train_count:
            max_train_count = train_cnt[term]
    print("train_max_pop:", max_train_count)
...
def split_unbias_TO2(unbias_path=''):  # 将unbias_data划分为valid set和test set  1:1  (key)
    ...
    testset = pd.DataFrame(
        columns=('user_id:token', 'item_id:token', 'rating:float', 'timestamp:float','wday:float'))
    data = pd.read_csv(unbias_path, header=0,
                       names=['user_id:token', 'item_id:token', 'rating:float', 'timestamp:float','wday:float'], sep='\t')
   # print('data', data)
